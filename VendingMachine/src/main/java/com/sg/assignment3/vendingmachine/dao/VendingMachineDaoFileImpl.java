...
public class VendingMachineDaoFileImpl implements VendingMachineDao{
   ...
   // no-arg constructor
    ...
    public void updateItemsInTheListinDao(int index) throws VendingMachinePersistenceExeption {
        if(listOfAllItems.get(index).getNumOfItemsInInventory() >= 0) { // no update for index == -1
    ...
    // process user entered money
    @Override
    public boolean processMoneyEntered(double userMoney, int selection) throws InsufficientFundException {
        if (userMoney < listOfAllItems.get(selection).getItemCost())
            throw new InsufficientFundException("Money entered is not enough");
        else
            return true;
    ...
    // if the item is out, it throws NoItemInventoryException back to the service layer.
    // Otherwise, it returns false to the service layer
    @Override
    public boolean checkItemLevelInTheStock(int itemId) throws NoItemInventoryException {

        if(listOfAllItems.get(itemId).getNumOfItemsInInventory() <= 0){
            throw new NoItemInventoryException(listOfAllItems.get(itemId).getItemName() + " is out of stock");
        }
